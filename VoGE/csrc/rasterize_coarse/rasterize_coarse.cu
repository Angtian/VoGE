...
__global__ void EllipseBoundingBoxKernel(
    const float* points, // (P, 3)
    const float* radius, // (P, 2)
    const int P,
    float* bboxes, // (4, P)
    bool* skip_points) {
  ...
  for (int p = tid; p < P; p += num_threads) {
    ...
    const float rx = radius[p * 2 + 0];
    const float ry = radius[p * 2 + 1];
  ...
  for (int chunk = blockIdx.x; chunk < num_chunks; chunk += gridDim.x) {
    ...
    for (int byx = threadIdx.x; byx < num_bins_y * num_bins_x;
      ...
      if (start + count > M) {
        ...
        const char* warning = "Bin size was too small";
        printf(warning);
        continue;
      }
...
at::Tensor RasterizeEllipseCoarseCuda(
  ...
  EllipseBoundingBoxKernel<<<blocks, threads, 0, stream>>>(
